#!/usr/bin/env bash
# glibc動的リンクバイナリ（cargo-zigbuildでglibc 2.17 ABIに固定）からAppImageを組み立てる。
# 前提: make appimage-build 等で
#       target/x86_64-unknown-linux-gnu/release/nekoviewer がビルド済みであること
#       （このスクリプト自体はビルドを行わない）。
set -euo pipefail

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
PKG_DIR="$ROOT_DIR/packaging/appimage"
BIN_PATH="$ROOT_DIR/target/x86_64-unknown-linux-gnu/release/nekoviewer"
APPDIR="$ROOT_DIR/target/appimage/Nekoviewer.AppDir"
OUT_DIR="$ROOT_DIR/target/appimage"
APP_ID="io.github.Omanjusan.Nekoviewer"

if [ ! -f "$BIN_PATH" ]; then
    echo "エラー: $BIN_PATH が無い。先に 'make appimage-build' でビルドしてください" >&2
    exit 1
fi
...
cp "$PKG_DIR/$APP_ID.desktop" "$APPDIR/$APP_ID.desktop"
cp "$PKG_DIR/$APP_ID.png" "$APPDIR/$APP_ID.png"
