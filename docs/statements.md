...
　これらのイベントは読み込み途中・画像描画途中であっても受け付けてイベント処理を優先させる。キャンセルされた処理については終了させること。また、ワーカーが発行されているならキャンセルイベント受取から終了までを行うルートを整備しておくこと。
...
- 読み込み中はグレーカード表示だが、画像読み込み完了時にこの画像に差し替えられない
